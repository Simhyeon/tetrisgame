...
use crate::component::dyn_block::DynamicBlock;
use crate::config::{MovementBindingTypes, AxisBinding};

const INPUTINTERVAL: f32 = 0.08;
...
impl<'s> System<'s> for KeyInputSystem {
    ...
    fn run(&mut self, (mut locals ,blocks, input, time): Self::SystemData) {
        if let Some(mut timer) = self.key_interval.take(){
            ...
            for (local, _block) in ( &mut locals, &blocks).join(){
